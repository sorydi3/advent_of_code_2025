        ...
        .fold::<Vec<Vec<String>>, _>(convert_to_usize(&init_fold), |acc, row| {
            ...
            let res = acc
                ...
                .map(|c| {
                    ...
                    let res = value_acc.iter().zip_longest(&value_row).map(|res| {
                       let res =  [res.as_ref().left().unwrap_or(&&"".to_string()),res.as_ref().right().unwrap_or(&&"".to_string())].to_vec().iter().map(|c|c.to_string()).collect::<String>();
                       res
                    }).collect::<Vec<_>>();
